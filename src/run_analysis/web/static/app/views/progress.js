...
export async function renderProgress() {
  ...
  const qualityRows = (progress.quality_performance ?? []).map((item) => `<li><a href="#run/${item.activity_id}">${dateLabel(item.start_time)}</a><strong>${number(item.duration_minutes, 1)} min${Number.isFinite(item.pace_min_mile) ? ` · ${pace(item.pace_min_mile)}` : ""}</strong><small>${titleCase(item.workout_type)}${Number.isFinite(item.average_hr_bpm) ? ` · ${number(item.average_hr_bpm, 0)} bpm average` : ""}${Number.isFinite(item.distance_miles) ? ` · ${number(item.distance_miles, 2)} mi` : ""}</small></li>`).join("");
  view.innerHTML = `
    <section class="page">
      ...
      ${qualityRows ? `<article class="wide-card"><div class="card-heading"><div><p class="eyebrow">Quality work tracked separately</p><h2>Completed work blocks</h2></div></div><p>These blocks show quality-work execution. Warm-up evidence may inform the aerobic trend, while the work and cooldown cannot distort its pace/HR estimate.</p><ul class="analysis-metrics">${qualityRows}</ul></article>` : ""}
